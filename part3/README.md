...
Next, install node-red-node-pi-neopixel prerequisites
...
Open a browser on your laptop to the IP Address of the Raspberry Pi mesh node. Node-RED is running on Port 1880 [http://pi_IP_address:1880](http://pi_IP_address:1880)
...
The Quickstart test flow uses an **Inject** node on a 10 second timer to query the temperature and humidity from the DHT sensor, formats the data using a **Template** node (or a **Change** node) and then, using an **Watson IoT** node, sends the data to the Watson IoT Quickstart service.  Import this solution flow
  ...
  **Get the Code** : [Node-RED Watson IoT Quickstart Test Flow](flows/quickstart-test-flow.json)
